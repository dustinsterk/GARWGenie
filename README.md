...
controller — all from your laptop, over the device's own Wi‑Fi. (Settings live in `~/.garw_genie/`;
older folders are migrated automatically. Artwork in `assets/` is generated by `assets/make_logo.py`
from the Press Start 2P glyphs — edit its PALETTE or FONT and re-run to change it; PNG, SVG and
.ico are regenerated.)
...
temperature and more, plus install layout, bootloader, whether the IC7 process is running, OS,
kernel, Qt, CPU model / cores / clock / temperature, RAM, storage, uptime and load, clock, IP/MAC,
and whether `ccrypt` is present. **Install firmware…** takes an official GARW update package (the
`.zip` you would otherwise put on a USB stick) and installs it the same way the stick would. This is
also how a v4 device gets to v5. The package is uploaded to `/mnt` on the unit (the stock updater's
...
`ccrypt` is used instead and the plain tar uploaded. The v4→v5 package leaves the old
`/opt/Garw_IC7` in place; the tool offers to remove it after a successful update so the unit is in the
clean v5 layout.
